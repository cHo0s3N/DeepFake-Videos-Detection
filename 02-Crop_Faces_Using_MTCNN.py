...
# The path of the dataset contains all the converted frames for the videos
...
# Processing Directory && Creating folder under the name of faces for each video:
for filename in metadata.keys():
    ...
    for frame in frame_images:
        ...
        detector = MTCNN()   # MTCNN is a strong face detector offering high detection scores. It stands for Multi-task Cascaded Convolutional Networks.
        image = cv2.cvtColor(cv2.imread(os.path.join(tmp_path, frame)), cv2.COLOR_BGR2RGB) #method is used to convert an image from one color space to another.
        results = detector.detect_faces(image)
        print('Face Detected: ', len(results))
        count = 0
        
        # Use Bounding Box to highlight the region of the face and how much our bounding create a confidence value 
